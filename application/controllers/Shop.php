...
class Shop extends MY_Controller {
    ...
    public function sortby($sort, $page = null){
        ...
        $data['content']    = $this->shop->select(
            ['product.id', 'product.title AS product_title', 'product.image', 'product.description', 'product.price', 'product.is_available', 'category.title AS category_title', 'category.slug AS category_slug']
        )
        ->join('category')
        ...
        $data['pagination'] = $this->shop->makePagination(
            base_url("shop/sortby/$sort"), 2, $data['total_rows']
        );
    ...
    public function category($category, $page = null){
        ...
        $data['pagination'] = $this->shop->makePagination(
            base_url("shop/sortby/$category"), 4, $data['total_rows']
        );
    ...
    public function search($page = null){
        ...
        $data['pagination'] = $this->shop->makePagination(base_url('product/search'), 3, $data['total_rows']);
        $data['page']       = 'pages/home/index';
